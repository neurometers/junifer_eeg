...
class PermutationEntropy(BaseMarker):
    ...
    _MARKER_INOUT_MAPPINGS: ClassVar = {
        "EEG": {"permutation_entropy": "vector"}
    }
    ...
    def compute(
        ...
        if signal_sym_length <= 0:
            raise ValueError(
                f"Signal too short for kernel={self.kernel} and tau={self.tau}. "
                f"Need at least {self.tau * (self.kernel - 1) + 1} samples."
            )

        signal_sym = np.zeros((n_channels, signal_sym_length), dtype=np.int32)
        ...
        for ch in range(n_channels):
            ...
            pe_values[ch] = pe / np.log(n_symbols)

        # Return data in junifer format
        return {
            "permutation_entropy": {
                "data": pe_values.reshape(1, -1),  # Shape: (1, n_channels)
                "col_names": [
                    f"{ch}_permutation_entropy" for ch in raw.ch_names
                ],
            }
        }
